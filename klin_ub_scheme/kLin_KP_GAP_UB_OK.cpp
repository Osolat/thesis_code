...
int main(int argc, char **argv) {
    ...
    for (int jo = 0; jo < 1; jo++) {
        ...
        for (int d = 0; d < (two_k * kss); ++d) {
            ...
            g2_mul_fix(mpk.A1_mat[d], t_pre_h, A1_tmp[d]);
        ...
        for (int k = 0; k < (kss * kss); k++) {
            ...
            g2_mul_fix(mpk.AW_mat[k], t_pre_h, AW[k]);
            g2_mul_fix(mpk.AW0_mat[k], t_pre_h, AW0[k]);
            g2_mul_fix(mpk.AW1_mat[k], t_pre_h, AW1[k]);
    ...
    for (int no = 0; no < NTESTS; no++) {
        ...
        for (auto it = res.begin(); it != res.end(); ++it) {
            ...
            for (int k = 0; k < (kss); k++) {
                ...
                g1_mul_fix(sk.sk13[it->leaf_index - 1].sk_two[k], t_pre_g, vj.rj[it->leaf_index - 1].vec_rj[k]);
            }
            ...
            for (int s = 0; s < (two_k); ++s) {
                g1_mul_fix(sk.sk13[it->leaf_index - 1].sk_three[s], t_pre_g, W0_w1_rj[s]);
                g1_mul_fix(sk.sk13[it->leaf_index - 1].sk_one[s], t_pre_g, v_plus_w[s]);
            }
        }
            ...
            for (auto it5 = res.begin(); it5 != res.end(); ++it5) {
                if (i == 0) {
                    ...
                    for (int y = 0; y < two_k; ++y) {
                        g1_mul(full_pairings_g1[y + c + kss], sk.sk13[it5->leaf_index - 1].sk_three[y], it5->coeff);
                        g1_neg(full_pairings_g1[y + c + kss], full_pairings_g1[y + c + kss]);
                        g2_copy(full_pairings_g2[y + c + kss], CT_A.C_23[it5->leaf_index - 1].c_3_vec[y]);
                    }
                    c += two_kk;
                }
                d = c + i;
