...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < two_k; ++i) {
        ...
        //init_null_new_g1_t_var(K4_prod[i]);
        init_null_new_g1_t_var(sk1_tmp[i]);
        //init_null_new_g1_t_var(sk4_tmp[i]);
        g1_set_infty(K1_prod[i]);
            ...
            for (auto it5 = res.begin(); it5 != res.end(); ++it5) {
                if (i == 0) {
                    ...
                    for (int z = 0; z < two_k; ++z) {
                        if (z < kss) {
                            g1_mul(full_pairings_g1[z + c], sk.sk13[it5->leaf_index - 1].sk_two[z],it5->coeff);
                            g2_copy(full_pairings_g2[z + c], CT_A.C_23[it5->leaf_index - 1].c_2_vec[z]);
                            a++;
                        }
                        //TODO could use pack_coef_neg instead. Faster than g1_neg???
                        g1_mul(full_pairings_g1[z + c + kss], sk.sk13[it5->leaf_index - 1].sk_three[z],it5->coeff);
                        g1_neg(full_pairings_g1[z + c + kss], full_pairings_g1[z + c + kss]);
                        g2_copy(full_pairings_g2[z + c + kss], CT_A.C_23[it5->leaf_index - 1].c_3_vec[z]);
                        b++;
                    }
                    c = b + a;
                }
